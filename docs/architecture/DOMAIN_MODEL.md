...
Enforcement status update (issue #9): `crates/openstream-engine` now implements the action-graph subset of §5–§6 — immutable validated graphs constructed only through stages S1–S4 (unique bounded keys, resolvable typed edges with source-kind-derived semantics, single-parent tree discipline, acyclicity over flow ∪ compensation edges, reachability, node limit 128 and container-depth 16, macro deadline caps, registry-referential capability scopes, retry-idempotency and compensate-policy semantics); a typed action REGISTRY declaring capability scopes (the manifest layer), idempotency class, and safe-compensation per adapter; and a deterministic discrete-event runtime enforcing journal-first admission (durable dedupe/`accepted` before effects, `prepared` before every dispatch, terminal evidence after, refused writes fail closed before dispatch), envelope expiry at admission only (expired commands journal `expired` and never queue), admission dedupe over `(source_device_id, message_id)` with ADR-0005 retention bounds and `outcome_unknown`-exempt oldest-first pruning, monotonic deadlines (default 30 s, macro cap 10 min) with node overrides, concurrency caps (4 per adapter / 32 global), cancellation propagation, failure policies stop/continue/compensate, deterministic-backoff retry gated on declared idempotency, crash-gap recovery closing orphaned prepared records as `outcome_unknown`, and idempotency-gated replay linking the original execution id. All timing derives from an injected clock (`FakeClock` ships; no system-clock implementation until #16); persistence stays behind the journal trait port (SQLite is #15); concrete OS/OBS adapters are #10–#13; sequencing features beyond v1 node semantics remain #14.

Enforcement status update (issue #14): the v1 multi-action semantics above are now proven end-to-end against the real registered adapters — dedicated integration suites drive sequence/parallel/delay/conditional/retry/compensate graphs over the shipped keyboard, launch, media/volume (`integrations/os-automation/tests/graph_semantics_integration.rs`) and OBS (`integrations/os-obs/tests/graph_semantics_integration.rs`) action types behind their recorded fakes, asserting exact virtual-time scheduling, stop/continue policy behavior, execution deadlines across delay nodes, cancellation propagation, typed adapter failure codes, the destructive arming gate in graph context, and the idempotency/compensation hard rules refused at validation against the adapters' honest declarations (retry of a non-idempotent action and `compensate` policy without safe-compensation proof both reject before any dispatch). A residual-work settlement rule was added to the runtime: a terminal decided mid-pass under `stop` or an unknown outcome resolves every sibling in-flight effect and queued slot waiter exactly once, so no open crash-gap window survives a persisted terminal and `recover_outcome_unknown()` can never overwrite a decided terminal.

Enforcement status update (issue #19): §3's Profile entity now carries optional explicit switch rules (additive-minor field, `serde(default)`, pre-#19 documents decode unchanged) and the switching model is implemented in `crates/openstream-domain/src/switching.rs` — closed-vocabulary triggers (global hotkey combos with mandatory modifiers, exact focused-app identities), deterministic workspace-wide conflict rejection of duplicate triggers, documented batch precedence where explicit hotkeys outrank focused-app automation and denials never rewind authorized switches, and two additive reserved-domain capabilities (`os.hotkey.register`, `os.focus.read`, ADR-0006) behind per-mechanism first-use consent with immediate revocation semantics. The desktop composition (`apps/desktop/src-tauri/src/{hotkeys,focus,switching}.rs`) enforces the registration-only/no-capture boundary and visible degradation contract; full behavioral detail lives in `docs/architecture/PROFILE_SWITCHING.md`.
