...
Adapter-side rows above (OBS, keyboard/media, launch/process, clipboard, filesystem, network, MIDI/OSC, notifications): `os.keyboard.emit` is enforced per its paragraph above, `os.application.launch` joins it per the issue #11 paragraph above, and `os.media.emit` (transport scope) plus `audio.control:device=master` join them per the issue #12 paragraph above; the remaining rows remain contracts for their named milestone issues.
