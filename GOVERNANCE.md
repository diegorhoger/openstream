...
- Repository-owner PR comments relay exactly one machine-readable evidence record per required role, bound to the body context and exact head, with commands, structured result/output digests, and a reviewer-report digest. The PR body names four distinct comment IDs; the trusted default-branch gate fetches only those comments. Owner comment mutation and relevant PR lifecycle/body changes republish a SHA-bound evidence status.
